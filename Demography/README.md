...
To not have a biased estimators, it’s better to have minimum 10 samples
([paper here](https://doi.org/10.1002/ece3.1846)).
...
Version: bcftools v1.10.2, ANGSD v0.940, R v4.2, vcftools v0.1.16,
Relate v1.1.9.

First, I have to separate each chromosome, “phasing”, and create
hap/sample/legend files.
...
The mask file is created based on the coregenome as defined by [Daron et
*al.* (2017)](0.1126/sciadv.abc3713).

Create pop labels file (space delimited), with 4 columns: ID POP GROUP
SEX
...
The input files are from Relate. First we have to crate a `mut` file
with fixed column file.
